...
2. 使用go.mod设置环境变量 
   ...
   1. 设置代理：`GOPROXY=https://goproxy.cn,direct`, 并重启ida
   2. 配置第三方包 `go mod tidy`
   2. 编译 `go build .\control.go`


3. 本次开发都是使用的第一种方法（借的书太老了，写的都是旧知识）



### 服务端的使用

1. 启动客户端control.exe，然后进行初始化，之后就可以等待测试客户端Clnt0.exe的连接
...
5. 然后进入主机选项界面，主要有一下几个功能：
   ...
   * 指令5是文件查看，进入后就可以选择盘符，统一使用 “/” 作为分节符，例如进入D盘就可以输入 `cd C:`
     * 进入c盘就会列出c盘的目录，然后可以继续进入下一个文件夹：`cd windows`
     * 返回上一级目录可以使用 ` cd ..`
     * 如果需要 **当前**文件夹下某个文件，只需要输入get 文件名称即可，例如`get 1.txt`
     * 当要给**当前文件夹**发送文件的时候，就需要使用 put指令，输入put指令后需要输入本地要上传文件的路径 **使用绝对路径**例如发送本地文件 C:/windows/1.txt之后就可以发送文件了  
...
1. 启动程序后，会先进行初始化，包括新建文件夹，监听端口。
   1. 之后会启动监听协程，一旦有客户端连接，就会初始化一个map来存储这个连接的信息，包括这个连接的用户信息，通信通道等等。
   2. 创建完map后，就会为这个连接启动 信息接受协程，信息处理协程，信息发送协程，并向客户端发送whoami来确认连接的建立并唤醒客户端。
   3. 当客户端返回whoami信息的时候，就会将这个消息发送到主程序上，让用户可见，并告知新的主机上线了。
   4. 客户端和服务端的心跳检测是不被用户所感知的，但是可以通过每次返回到主界面来观察与客户端的最后一次心跳，任何一次信息交换都视作一次心跳，且无交换时，客户端也会每60s向服务端发送存活通知。
   4. 协程之间的联系是通过客户结构体中的各个管道交换信息的。
...
4. 在文件操作的时候，只需要输入1就可以查看文件操作帮助
   ...
   1. 文件操作包括`cd` ` dir` `put` `get` `del（未上线）` 
   2. 其本质时无状态有会话的一个过程，访问记录又服务端记录，用户输入的相对路径会在后台处理成绝对路径然后发送给客户端，客户端再相应的返回信息，这个过程中对客户端要求很高，所以也是本次合作中的难点之一，即便时花费了一个月的时间，我们也无法确保很大很大的文件传输顺利。
...
*  主程序，所属main包初始化各个文件夹，启动监听，并调用各个协程
* 用户交互界面，各个命令生成的地方，发送用户的输入和接受输出，通过各个管道与各个协程通信
...
* 所有发送来的消息都会率先进入此包，它会确保消息完整，并交由解密包解密，最终通过消息处理协程将文件发送到合适的地方，或是终端或是文件
...
* 进行加密前的base64编码，确保文件的可传输性
* 处理所有发送文件的加密和接受的所有文件的解密
* 并且预留了一些常用来转换字符的方法函数以便于以后进行扩展
...
* 加密随机性不足，后期可以使用更为先进的加密方式，但是在时间范围较大的情况下也是又足够的随机性。

* 注释可能不太多，建议看完readme后，玩几遍程序，再看源代码

  

### 其他

如何在windows上关闭客户端（指前台客户端没了，但是实际上还存在在电脑上运行的时候）

1. 让客户端和服务端连接
2. 然后输入指令`netstat -aon|findstr "6666"` 找到占用6666端口的程序pid
3. 然后终止程序` taskkill /T /F /PID 此处写pid `即可关闭客户端
